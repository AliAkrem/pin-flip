...
class PinFlipLineChart extends StatelessWidget {
  const PinFlipLineChart({super.key, this.events = const <DetailedEventData>[]});
  ...
  Widget build(BuildContext context) {
    return CustomPaint(
      painter: PinFlipLineChartPainter(
        dateFormat: dateFormatMonthYear(context),
        numberFormat: usdWithSignFormat(context),
        events: events,
        labelStyle: Theme.of(context).textTheme.bodyMedium!,
        textDirection: PinFlipOptions.of(context).resolvedTextDirection()!,
...
class PinFlipLineChartPainter extends CustomPainter {
  PinFlipLineChartPainter({
  ...
  void _drawLine(Canvas canvas, Rect rect) {
    final linePaint = Paint()
      ..color = PinFlipColors.accountColor(2)
